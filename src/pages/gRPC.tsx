...
export async function getBalances(adder: any, coinList: any) {
    const query = `
    query Balances($owner: SuiAddress!, $sui: String!, $deep: String!, $wal: String!) {
      address(address: $owner) {
        multiGetBalances(keys: [$sui, $deep, $wal]) {
          coinType { repr }
          totalBalance
        }
      }
      suiMeta:  coinMetadata(coinType: $sui)  { symbol decimals }
      deepMeta: coinMetadata(coinType: $deep) { symbol decimals }
      walMeta:  coinMetadata(coinType: $wal)  { symbol decimals }
    }
  `;
    const variables = { ...adder, ...coinList };
    ...
    // 关键：把 Response 解析成 JSON
    const { data, errors } = await res.json();
    if (errors) throw new Error(JSON.stringify(errors, null, 2));
    // 读具体余额（链上整数，string）
    const bianList = [];
    for (const b of data.address.multiGetBalances) {
        const raw = Number(b.totalBalance); // 大整数
        bianList.push(raw);
    }
    return bianList;
}
